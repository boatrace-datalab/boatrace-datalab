    ...
    for line in lines:
        ...
        if m:
            if current_race_no and rank_boats:
                _save_race(races, results, race_date_str, current_venue_code,
        ...
        if in_data and current_venue_code and current_race_no:
            ...
            if m:
                rank_boats.append((int(m.group(1)), int(m.group(2))))
...
def _save_race(races, results, race_date_str, venue_code, race_no,
...
def save_to_supabase(pg_conn, races, results):
    if not races:
        return 0
...
def process_date(dt, pg_conn):
    ...
    if not text:
        return 0
    races, results = parse_k_file(text, race_date_str)
    log.info(f"  パース: {len(races)}レース")
    saved = save_to_supabase(pg_conn, races, results)
